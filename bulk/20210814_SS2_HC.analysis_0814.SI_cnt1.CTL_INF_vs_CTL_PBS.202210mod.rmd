...
p1.1 = p1.1 + geom_point(size=3.5, alpha=.9)
...
p1.1 = p1.1 + labs(title="PCA ",
               color="Condition", shape="batch")
p1.1 = p1.1 + scale_colour_manual(values = cols, limits=rev) + theme_classic()
#p1.1 = p1.1 + scale_shape_manual(values=c(16,17,15))
p1.1
```

```{r eval=FALSE, include=FALSE}
ggsave("./figures/SI_cnt1.CTL_INF_vs_CTL_PBS/fig.S4C.PCA.pdf",
       plot = p1.1,
       width = 4.2, height = 3)
```
...
```{r eval=FALSE, include=FALSE}
ggsave("./figures/SI_cnt1.CTL_INF_vs_CTL_PBS/fig.S4C.tSNE.pdf",
       plot = p6.1,
       width = 4.2, height = 3)
```
...
ret <- run_MEedgeR(MAT = mat_pc[,c(Aidx,Bidx)], ED = Aname, WT = Bname, n1 = length(Aidx),n2 = length(Bidx),
                         lcm = 0, padj = 0.05,lfc = log2(2), gsea = TRUE)
```
...
rets <- finalplot(mat_pc[,c(Aidx,Bidx)],data.frame(ret$DEG_edgeR), paste0(Aname,"_vs_",Bname), 0.05,2, 
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
rets1 <- finalplot(mat_pc[,c(Aidx,Bidx)],data.frame(ret$DEG_edgeR), paste0(Aname,"_vs_",Bname), 0.05, 2, 
                  Sign = FALSE, Sign_dn = 40, Sign_up = 40, padjust = TRUE, Label = "")
```

```{r echo=FALSE, fig.height=4, fig.width=6.1, warning=FALSE}
rets1$vol
```


```{r eval=FALSE, include=FALSE}
ggsave("./figures/SI_cnt1.CTL_INF_vs_CTL_PBS/fig.S4D.volcano.FC2.pdf",
       plot = rets1$vol,
       width = 6.4, height = 4)
```
...
pheatmap::pheatmap(log2(mat_pc[genes,c(rev(Bidx),rev(Aidx))]+1),cluster_rows = F, cluster_cols = F,
                   fontsize_row = 7.6,
...
```{r echo=FALSE, fig.height=9.6, fig.width=4.8}
pheatmap::pheatmap(zscore_mat(log2(mat_pc[genes,c(rev(Bidx),rev(Aidx))]+1)),cluster_rows = F, cluster_cols = F,
         fontsize_row = 7.6,
         gaps_col = length(Bidx),
         gaps_row = 40, color = color.test,border_color = NA,
         breaks = seq(-1.8,1.8,0.036),
         main = "top zscore")
...
```{r eval=FALSE, include=FALSE}
write.table(rec_gcol(data.frame(ret$DEG_edgeR)), paste0("edgeR_DEGs.SI_",
...
write.table(proc.mat(Mat=ret$TMM$counts[rownames(ret$DEG_edgeR),]),
            paste0("GSEA_mit/RNAseq.",mtxname,".SI_",Aname,"_vs_",Bname,".filt_tmm.pc_gene.txt"),
            col.names = TRUE, row.names = FALSE, sep = "\t", quote = FALSE)

write.table(c(paste0(length(c(Aidx,Bidx))," 2 1"),
              paste0("# ",Aname," ",Bname),
              paste(c(rep("1",length(Aidx)),rep("0",length(Bidx))),collapse = " ")),
            paste0("GSEA_mit/RNAseq.",mtxname,".SI_",Aname,"_vs_",Bname,".cls"),
...
```{r echo=FALSE, fig.height=9.6, fig.width=4.8}
#genes <- rets$marker
genes1 <- c(rets$down[1:169],rev(rets$up[1:163]))

pheatmap::pheatmap(log2(mat_pc[genes1,c(Bidx,Aidx)]+1),cluster_rows = F, cluster_cols = F,
                   fontsize_row = 1.2, show_rownames = F,
         gaps_col = length(Bidx), border_color = NA,
         gaps_row = 169,
         main = "top log2(CPM+1)")
...
```{r echo=FALSE, fig.height=9.6, fig.width=4.8}
pheatmap::pheatmap(zscore_mat(log2(mat_pc[genes1,c(rev(Bidx),rev(Aidx))]+1)),cluster_rows = F, cluster_cols = F,
         fontsize_row = 1.6, show_rownames = F,
         border_color = NA,
         gaps_col = length(Bidx),
         gaps_row = 169, color = color.test,
        breaks = seq(-2,2,0.04),
         main = "top zscore")
...
```{r eval=FALSE, include=FALSE}
pdf("./figures/SI_cnt1.CTL_INF_vs_CTL_PBS/fig.S4E.heat_small.pdf",
    width = 4.8, height = 9.6)
pheatmap::pheatmap(log2(mat_pc[genes,c(rev(Bidx),rev(Aidx))]+1),cluster_rows = F, cluster_cols = F,
                   fontsize_row = 7.6,
         gaps_col = length(Bidx), border_color = NA,
         gaps_row = 40,
         main = "top log2(CPM+1)")
dev.off()
pdf("./figures/SI_cnt1.CTL_INF_vs_CTL_PBS/fig.S4E.heatz_small.pdf",
    width = 4.8, height = 9.6)
pheatmap::pheatmap(zscore_mat(log2(mat_pc[genes,c(rev(Bidx),rev(Aidx))]+1)),cluster_rows = F, cluster_cols = F,
         fontsize_row = 7.6,
         gaps_col = length(Bidx),
...
#
pdf("./figures/SI_cnt1.CTL_INF_vs_CTL_PBS/fig.S4E.heat_big.pdf",
    width = 4.8, height = 9.6)
pheatmap::pheatmap(log2(mat_pc[genes1,c(rev(Bidx),rev(Aidx))]+1),cluster_rows = F, cluster_cols = F,
                   fontsize_row = 1.6, show_rownames = T,
         gaps_col = length(Bidx), border_color = NA,
         gaps_row = 169,
         main = "top log2(CPM+1)")
dev.off()
pdf("./figures/SI_cnt1.CTL_INF_vs_CTL_PBS/fig.S4E.heatz_big.pdf",
    width = 4.8, height = 9.6)
pheatmap::pheatmap(zscore_mat(log2(mat_pc[genes1,c(rev(Bidx),rev(Aidx))]+1)),cluster_rows = F, cluster_cols = F,
         fontsize_row = 1.6, show_rownames = T,
         border_color = NA,
