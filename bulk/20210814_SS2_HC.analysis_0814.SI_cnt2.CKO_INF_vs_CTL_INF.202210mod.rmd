...
```{r eval=FALSE, include=FALSE}
ggsave("./figures/SI_cnt2.CKO_INF_vs_CTL_INF/fig.5A.PCA.pdf",
       plot = p1,
       width = 4.2, height = 3)
```
...
```{r eval=FALSE, include=FALSE}
ggsave("./figures/SI_cnt2.CKO_INF_vs_CTL_INF/fig.5A.tSNE.pdf",
       plot = p6.1,
       width = 4.2, height = 3)
```
...
```{r eval=FALSE, fig.height=6, fig.width=8, warning=FALSE}
rets$vol
```
...
rets <- finalplot(mat_pc[,c(Aidx,Bidx)],data.frame(ret$DEG_edgeR), paste0(Aname,"_vs_",Bname), 0.05, 1.5, 
                  Sign = FALSE, Sign_dn = 40, Sign_up = 40, padjust = FALSE)
```
...
```{r echo=FALSE, fig.height=3.5, fig.width=5.6, warning=FALSE}
rets1$vol
```


```{r eval=FALSE, include=FALSE}
ggsave("./figures/SI_cnt2.CKO_INF_vs_CTL_INF/fig.S5A.volcano.pdf",
       plot = rets1$vol,
       width = 6, height = 3.5)
```
...
```{r echo=FALSE, fig.height=7.6, fig.width=4}
#genes <- rets$marker
genes <- c(rets$down[1:23],rev(rets$up[1:16]))
...
pheatmap::pheatmap(zscore_mat(log2(mat_pc[genes,c(rev(Bidx),Aidx)]+1)),cluster_rows = F, cluster_cols = F,
         ...
         gaps_row =23, color = color.test,border_color = NA,
         breaks = seq(-2,2,0.04),
         main = "top zscore")
...
```{r eval=FALSE, include=FALSE}
pdf("./figures/SI_cnt2.CKO_INF_vs_CTL_INF/fig.5B.heat.pdf",
    width = 4, height = 7.6)
pheatmap::pheatmap(log2(mat_pc[genes,c(rev(Bidx),Aidx)]+1),cluster_rows = F, cluster_cols = F,
                   fontsize_row =8,
         gaps_col = length(Bidx), border_color = NA,
...
dev.off()
pdf("./figures/SI_cnt2.CKO_INF_vs_CTL_INF/fig.5B.heatz.pdf",
    width = 4, height = 7.6)
pheatmap::pheatmap(zscore_mat(log2(mat_pc[genes,c(rev(Bidx),Aidx)]+1)),cluster_rows = F, cluster_cols = F,
         fontsize_row =8,
         gaps_col = length(Bidx),
